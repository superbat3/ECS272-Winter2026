...
{// Read in csv files
    const dataFromCSV = await d3.csv('./data/spotify.csv', (d) => {
        ...
        return { track_id: d.track_id, track_name: d.track_name, track_popularity: +d.track_popularity,
            artist_name: d.artist_name}
        })
    bars = dataFromCSV.slice(0,5)
...
const onResize = (targets) => {
    targets.forEach(target => {
        ...
        if (size.width > 0 && size.height > 0 && bars.length > 0) {
...
function initChart() {
    ...
    let yExtents = d3.extent(bars.map((d) => d.track_popularity))
    // This is to get the unique categories from the data using Set, then store in an array.
    let xCategories = [...new Set(bars.map((d) => d.track_name))]
    ...
    const barEles = chartContainer.append('g')
        ...
        .attr('y', (d) => yScale(d.track_popularity))
        // specify the size of the rectangle
        .attr('width', xScale.bandwidth())
        .attr('height', (d) => Math.abs(yScale(0) - yScale(d.track_popularity))) // this substraction is reversed so the result is non-negative
        .attr('fill', 'teal')
    ...
    const title = chartContainer.append('g')
        ...
        .text('Distribution of Spotify Data') //text content
